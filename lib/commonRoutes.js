...
function serverID(req,res,next,serverID) {
    ...
    } else if (process.env['DEFAULT_ROUTE_API_URL']) {
...
function isAuthenticatedGuest(req,res,next) {
    if (req.query.key == process.env.API_KEY || req.query.key == process.env.GUEST_API_KEY) return next()
    res.status(401).send('Missing or invalid api key.')
    consoleLog('warning','auth',`Invalid key ${req.query.key} from <{ip}>`,req)
}
...
module.exports = {
    ...
    isAuthenticatedGuest: isAuthenticatedGuest
}